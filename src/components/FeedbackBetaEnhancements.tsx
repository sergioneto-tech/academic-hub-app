...
function ensureStyles() {
  ...
  style.textContent = `
    ...
    .ah-feedback-kind-counts {
      display: inline-flex;
      align-items: center;
      gap: .22rem;
      margin-left: auto;
    }
    .ah-feedback-kind-count {
      display: inline-grid;
      place-items: center;
      min-width: 1.15rem;
      height: 1.15rem;
    ...
    button[data-feedback-list-kind="opinion"] {
      border-left: 4px solid rgb(59 130 246) !important;
      background-image: linear-gradient(90deg, rgb(59 130 246 / .08), transparent 34%);
    }
    button[data-feedback-list-kind="suggestion"] {
      border-left: 4px solid rgb(34 197 94) !important;
      background-image: linear-gradient(90deg, rgb(34 197 94 / .08), transparent 34%);
    }
    button[data-feedback-list-kind="bug"] {
      border-left: 4px solid rgb(239 68 68) !important;
      background-image: linear-gradient(90deg, rgb(239 68 68 / .08), transparent 34%);
    }
    ...
    @media (max-width: 639px) {
      #${FILTER_ID} { grid-template-columns: minmax(0, 1fr); margin-inline: 1rem; }
      .ah-feedback-kind-counts { gap: .18rem; }
    }
...
function enhanceMenu() {
  ...
  document.querySelectorAll<HTMLAnchorElement>('a[href$="/feedback"]').forEach((link) => {
    ...
    let group = link.querySelector<HTMLSpanElement>(".ah-feedback-kind-counts");
    if (!count) {
      group?.remove();
      return;
    }
...
function enhanceInbox() {
  ...
  card.querySelectorAll<HTMLButtonElement>("button").forEach((button) => {
    ...
    const visibleType = activeType === "all" || entry.kind === activeType;
    const visibleStatus = activeStatus === "all" || entry.status === activeStatus;
    button.hidden = !(visibleType && visibleStatus);
  });
}
