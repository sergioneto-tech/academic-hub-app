...
function completedCourseRequirement(
  state: AppState,
  plan: PlanCourseSeed[],
  id: string,
  label: string,
  requiredCodes: string[],
): CourseRequirementStatus["requirements"][number] {
  ...
  return {
    id,
    label,
    met: missingCourses.length === 0 && requiredCourses.length === requiredCodes.length,
    detail: `${requiredCourses.length - missingCourses.length}/${requiredCourses.length} UC concluídas`,
...
const REQUIREMENTS: RequirementDefinition[] = [
  {
    ...
    evaluate: (state, plan) => {
      ...
      const areaEcts = plan.reduce((total, course) => {
        if (!completed.has(course.code) || !course.area || !eligibleAreas.has(course.area)) return total;
      ...
      return [
        {
          ...
          detail: `${firstYear.length - missingFirstYear.length}/${firstYear.length} UC concluídas`,
  ...
  },
  {
    degreeId: "uab-lca",
    courseCode: "21118",
    sourceUrl: "https://guiadoscursos.uab.pt/ucs/trabalhos-de-campo-i/",
    sourceLabel: "Guia dos Cursos UAb — Trabalhos de Campo I",
    note: "A UAb indica expressamente que a inscrição nesta UC está sujeita à aprovação prévia nas quatro UC indicadas.",
    evaluate: (state, plan) => [
      completedCourseRequirement(
        state,
        plan,
        "tc1-foundations",
        "Biologia Geral I e II + Geologia Geral I e II concluídas",
        ["21012", "21013", "21056", "21057"],
      ),
    ],
  },
  {
    degreeId: "uab-lca",
    courseCode: "21119",
    sourceUrl: "https://guiadoscursos.uab.pt/ucs/trabalhos-de-campo-ii/",
    sourceLabel: "Guia dos Cursos UAb — Trabalhos de Campo II",
    note: "A UAb refere que apenas se podem inscrever estudantes em posição de concluir o curso no respetivo ano letivo e indica, como referência, não ter mais de 5 a 8 UC por concluir. Situações especiais podem ser analisadas pela UAb.",
    evaluate: (state, plan) => {
      const completed = completedCodes(state);
      const remainingExcludingTarget = plan.filter((course) => course.code !== "21119" && !completed.has(course.code));
      return [
        {
          id: "tc2-course-completion-window",
          label: "Em posição de concluir a licenciatura neste ano letivo",
          met: remainingExcludingTarget.length <= 8,
          detail: remainingExcludingTarget.length === 0
            ? "Sem outras UC por concluir"
            : `${remainingExcludingTarget.length} UC por concluir além de Trabalhos de Campo II`,
          current: remainingExcludingTarget.length,
          target: 8,
          missingCourses: remainingExcludingTarget.slice(0, 8).map((course) => ({ code: course.code, name: course.name })),
        },
      ];
    },
  },
