...
function getEfolioPoints(text: string, label: string): number | null {
  ...
  const pattern = new RegExp(
    `e-?f[oó]lio\\s+${escapedLabel}[^\\n]{0,120}?(\\d+(?:[.,]\\d+)?)\\s*valores?\\b`,
    "i",
  );
  return parsePoints(text.match(pattern)?.[1]);
}

function pointsForEvent(result: PucParseResult, text: string, eventKey: string, eventName: string): number | null {
...
export function buildPucImportDraft(result: PucParseResult, rawText: string): PucImportDraft {
  const events = result.events
    ...
    .map((event) => ({
      key: event.key,
      name: event.name,
      maxPoints: pointsForEvent(result, rawText, event.key, event.name),
