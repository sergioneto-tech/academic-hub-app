    ...
    <div className={`academic-report-view academic-report-${orientation}`}>
      ...
      <section className="academic-report-sheet academic-report-page-two">
        ...
        {completed.length === 0 ? (
          ...
          <div className="academic-report-portrait-details">
            {reportRows.map(({ course, area, efolios, exam, resit, final }, index) => (
              <div className="academic-report-portrait-row" key={course.id}>
                <div className="academic-report-portrait-main">
                  <strong>{index + 1}. {course.name}</strong>
                  <span>{course.code} · {yearSemester(course)} · {area}</span>
                  <b>{finalGradeLabel(final)}</b>
                </div>
                <div className="academic-report-portrait-breakdown">
                  {efolios.map((item) => (
                    <span key={item.id}>{item.name}: <strong>{assessmentScore(item)}</strong></span>
                  ))}
                  <span>
                    {resit && resit.grade !== null ? "Recurso" : exam?.name || "Avaliação final"}: <strong>{resit && resit.grade !== null ? assessmentScore(resit) : assessmentScore(exam)}</strong>
                  </span>
                </div>
              </div>
            ))}
