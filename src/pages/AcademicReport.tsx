...
function ReportDisclaimer() {
  return (
    <div className="academic-report-disclaimer">
      ...
      <span>
        Documento informativo gerado pelo Academic Hub — não é um documento oficial da Universidade Aberta (UAb).
      </span>
    </div>
  );
}
...
export default function AcademicReportPage() {
  ...
  return (
    <div className="academic-report-view">
      ...
      <section className="academic-report-sheet academic-report-page-one">
        ...
        {completed.length === 0 ? (
          <div className="academic-report-empty">
      ...
      <section className="academic-report-sheet academic-report-page-two">
        ...
        {completed.length === 0 ? (
          <div className="academic-report-empty">
            Ainda não existem cadeiras concluídas para detalhar.
          </div>
        ) : (
          <div className="academic-report-table-wrap academic-report-detail-table-wrap">
            <table
              ...
              style={{ "--efolio-count": efolioColumns.length } as React.CSSProperties}
            >
              <thead>
                <tr>
                  <th>N.º</th>
                  ...
                  <th>Ano / Semestre</th>
                  <th>Categoria</th>
                  {efolioColumns.map((index) => (
                    <th key={index}>E-fólio {String.fromCharCode(65 + index)}</th>
                  ))}
                ...
                {reportRows.map(({ course, area, efolios, exam, resit, final }, index) => (
                  <tr key={course.id}>
                    ...
                    {efolioColumns.map((efolioIndex) => (
                      <td key={efolioIndex} className="academic-report-center">
                        {assessmentScore(efolios[efolioIndex])}
                      </td>
                    ))}
                    <td className="academic-report-center">
                      {resit?.grade !== null && resit
                        ? `R: ${assessmentScore(resit)}`
                        : assessmentScore(exam)}
                    </td>
                    <td className="academic-report-center academic-report-final-cell">
                      {final === null ? "—" : formatPtNumber(final)}
                    </td>
                  </tr>
                ))}
              </tbody>
