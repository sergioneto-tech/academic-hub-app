...
export default function CalendarPage() {
  ...
  const realActiveCourses = state.courses.filter((course) => course.isActive);
  const demoMode = realActiveCourses.length === 0;
  const displayCourses: DisplayCourse[] = demoMode ? DEMO_COURSES : realActiveCourses;
  const events = demoMode ? DEMO_EVENTS : realEvents;
  const today = localTodayYmd();
  const activeCourses = displayCourses.filter((course) => semester === "all" || String(course.semester) === semester);
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-center">
          ...
          <Select value={exportMode} onValueChange={(value) => setExportMode(value as typeof exportMode)} disabled={demoMode}>
          ...
          <Button variant="secondary" onClick={handleExportIcs} disabled={demoMode}><Smartphone className="mr-2 h-4 w-4" />Exportar (.ics)</Button>
