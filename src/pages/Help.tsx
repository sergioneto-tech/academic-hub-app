...
import { ArrowLeft, ArrowRight, BellRing, BookOpen, CalendarDays, CheckCircle2, Cloud, ExternalLink, FileText, Fingerprint, GraduationCap, HelpCircle, LayoutDashboard, LifeBuoy, LockKeyhole, MessageSquareText, Printer, Scale, Settings, ShieldCheck, Sparkles, UserPlus, UserRound } from "lucide-react";
...
export default function HelpPage(){
  ...
  return <div className="space-y-5">
    ...
    <section className="premium-surface overflow-hidden p-5 sm:p-7">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="max-w-2xl">
          ...
          <p className="mt-2 text-sm text-muted-foreground">Aprende a explorar, configurar e utilizar o Academic Hub, incluindo suporte, privacidade e controlo dos teus dados.</p>
        </div>
    ...
    <Card className="premium-card overflow-hidden border-[hsl(var(--gold)/0.45)]">
      <CardContent className="grid gap-0 p-0 xl:grid-cols-[minmax(0,1fr)_auto]">
        <div className="p-5 sm:p-6"><div className="flex items-start gap-3"><div className="grid h-12 w-12 shrink-0 place-items-center rounded-xl bg-[hsl(var(--gold-soft))] text-[hsl(var(--gold))]"><Sparkles className="h-6 w-6"/></div><div><div className="text-xs font-semibold uppercase tracking-wide text-[hsl(var(--gold))]">Novo no Academic Hub?</div><h2 className="mt-1 text-lg font-semibold">Como começar — guia passo a passo</h2><p className="mt-1 max-w-2xl text-sm text-muted-foreground">Percorre os principais passos desde a escolha do curso até às cadeiras, avaliações, calendário, notificações, histórico, suporte, privacidade e sincronização.</p></div></div></div>
        <div className="flex items-center border-t bg-[hsl(var(--gold-soft)/0.45)] p-4 xl:border-l xl:border-t-0"><Button className="w-full xl:w-auto" onClick={()=>{setStep(0);setOpen(true)}}>Começar guia <ArrowRight className="ml-2 h-4 w-4"/></Button></div>
      </CardContent>
    </Card>

    <Card className="premium-card overflow-hidden border-primary/25">
      <CardContent className="grid gap-0 p-0 xl:grid-cols-[minmax(0,1fr)_auto]">
        <div className="p-5 sm:p-6"><div className="flex items-start gap-3"><div className="grid h-11 w-11 shrink-0 place-items-center rounded-xl border bg-primary/5 text-primary dark:bg-primary/10"><LifeBuoy className="h-5 w-5"/></div><div><div className="text-xs font-semibold uppercase tracking-wide text-primary">Suporte e pedidos</div><h2 className="mt-1 text-lg font-semibold">Encontraste uma falha ou precisas de ajuda?</h2><p className="mt-1 max-w-2xl text-sm text-muted-foreground">Comunica erros, falhas, comportamentos inesperados ou dúvidas. Também podes enviar sugestões. Se estiveres autenticado, o pedido fica associado ao teu ID Academic Hub e podes acompanhar o respetivo estado.</p></div></div></div>
        <div className="flex items-center border-t bg-primary/5 p-4 xl:border-l xl:border-t-0"><Button asChild className="w-full xl:w-auto"><Link to="/feedback">Abrir Suporte <ArrowRight className="ml-2 h-4 w-4"/></Link></Button></div>
      </CardContent>
    </Card>

    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{guides.map(({title,description,icon:GuideIcon})=><Card key={title} className="premium-card h-full"><CardHeader className="pb-2"><div className="mb-2 flex h-10 w-10 items-center justify-center rounded-xl border bg-primary/5 text-primary dark:bg-primary/10"><GuideIcon className="h-5 w-5"/></div><CardTitle className="text-base">{title}</CardTitle></CardHeader><CardContent className="text-sm leading-6 text-muted-foreground">{description}</CardContent></Card>)}</div>

    <Card className="premium-card border-[hsl(var(--gold)/0.35)]"><CardHeader><CardTitle className="flex items-center gap-2 text-base"><UserRound className="h-5 w-5 text-[hsl(var(--gold))]"/>Sobre o projeto</CardTitle></CardHeader><CardContent className="space-y-2 text-sm text-muted-foreground"><p className="font-medium text-foreground">Criado e desenvolvido por Sérgio Neto, aluno da Licenciatura em Engenharia Informática da Universidade Aberta.</p><p>O Academic Hub nasceu como uma ferramenta pessoal de organização académica e evoluiu para uma aplicação independente de apoio aos estudantes.</p></CardContent></Card>

    <div className="grid gap-4 lg:grid-cols-2">
      <Card className="premium-card border-primary/25"><CardHeader><CardTitle className="flex items-center gap-2 text-base"><Scale className="h-5 w-5 text-primary"/>Privacidade, termos e segurança</CardTitle></CardHeader><CardContent className="space-y-4 text-sm text-muted-foreground"><p>Acede num só local à informação sobre tratamento de dados, Termos de Utilização, Segurança e Transparência, exportação, correção e eliminação.</p><Button asChild variant="outline"><Link to="/dados-privacidade">Abrir dados e privacidade</Link></Button></CardContent></Card>
      <Card className="premium-card border-primary/25"><CardHeader><CardTitle className="flex items-center gap-2 text-base"><Printer className="h-5 w-5 text-primary"/>Relatório académico pessoal</CardTitle></CardHeader><CardContent className="space-y-4 text-sm text-muted-foreground"><p>Imprime ou guarda em PDF o percurso registado, incluindo avaliações, nota final e ECTS.</p><Button asChild variant="outline"><Link to="/historico/relatorio">Abrir relatório</Link></Button></CardContent></Card>
    </div>

    <Card className="premium-card overflow-hidden border-[hsl(var(--gold)/0.45)]"><CardContent className="p-0"><div className="grid xl:grid-cols-[minmax(0,1fr)_auto]"><div className="p-5 sm:p-6"><div className="flex items-start gap-3"><div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-[hsl(var(--gold-soft))] text-[hsl(var(--gold))]"><FileText className="h-5 w-5"/></div><div><h2 className="font-semibold">Regulamento de avaliação atualmente publicado</h2><p className="mt-1 text-sm text-muted-foreground">Confirma sempre as regras aplicáveis no PUC e na informação oficial da Universidade Aberta.</p></div></div></div><div className="flex items-center border-t bg-[hsl(var(--gold-soft)/0.55)] p-4 xl:border-l xl:border-t-0"><Button asChild className="w-full xl:w-auto"><a href={REGULATION_URL} target="_blank" rel="noopener noreferrer">Consultar na UAb <ExternalLink className="ml-2 h-4 w-4"/></a></Button></div></div></CardContent></Card>

    <Card className="premium-card"><CardHeader><CardTitle className="text-base">Ligações úteis</CardTitle></CardHeader><CardContent className="grid gap-2 text-sm sm:grid-cols-2 lg:grid-cols-4"><a className="rounded-lg border p-3 hover:bg-accent" href="https://portal.uab.pt/" target="_blank" rel="noopener noreferrer">Portal UAb</a><a className="rounded-lg border p-3 hover:bg-accent" href="https://guiadoscursos.uab.pt/" target="_blank" rel="noopener noreferrer">Guia dos Cursos</a><a className="rounded-lg border p-3 hover:bg-accent" href="https://portal.uab.pt/calendario-letivo/" target="_blank" rel="noopener noreferrer">Calendário letivo</a><a className="rounded-lg border p-3 hover:bg-accent" href="https://www.dges.gov.pt/" target="_blank" rel="noopener noreferrer">DGES</a></CardContent></Card>
