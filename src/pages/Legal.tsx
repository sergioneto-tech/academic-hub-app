...
export default function LegalPage() {
  return <div className="space-y-5">
    ...
    <Card className="premium-card"><CardHeader><CardTitle className="text-base">Conservação dos dados</CardTitle></CardHeader><CardContent className="space-y-4"><p className="text-sm leading-6 text-muted-foreground">Os dados necessários à conta, sincronização e organização académica são conservados enquanto a conta se mantiver ativa ou até serem substituídos/eliminados pelo utilizador, salvo necessidade legal específica. Dados operacionais têm prazos próprios e limpeza automática.</p><div className="grid gap-2 md:grid-cols-2">{RETENTION.map(([label, period]) => <div key={label} className="flex items-start justify-between gap-4 rounded-xl border bg-card p-3"><span className="text-xs font-medium">{label}</span><span className="shrink-0 text-right text-xs text-muted-foreground">{period}</span></div>)}</div><p className="text-xs leading-5 text-muted-foreground">Se a conta for eliminada, os registos associados ao utilizador e os anexos privados de feedback são removidos pelo procedimento de eliminação, exceto dados que devam ser conservados temporariamente por obrigação legal ou defesa de direitos.</p></CardContent></Card>
