    ...
    <div className="space-y-6">
      ...
      <section className="premium-surface overflow-hidden p-5 sm:p-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          ...
          <Button asChild variant="outline" size="sm"><Link to="/seguranca-privacidade">Segurança e Privacidade</Link></Button>
        </div>
      </section>
      ...
      <Card className="premium-card">
        <CardHeader><CardTitle className="flex items-center gap-2 text-base"><Database className="h-4 w-4 text-primary"/>Que dados podem existir</CardTitle></CardHeader>
        <CardContent className="grid gap-3 text-sm text-muted-foreground sm:grid-cols-2 lg:grid-cols-3">
          {["Perfil e preferências da aplicação", "Licenciatura, cadeiras, datas e avaliações", "Dados de sincronização e estado da cloud", "Subscrições e registos operacionais de notificações", "Pedidos de feedback e anexos que envies", "Registos técnicos de erros e segurança necessários ao funcionamento"].map(item => <div key={item} className="rounded-xl border bg-muted/20 p-3 leading-5">{item}</div>)}
        </CardContent>
      </Card>

      <Card className="premium-card border-primary/20">
        <CardHeader><CardTitle className="flex items-center gap-2 text-base"><RefreshCw className="h-4 w-4 text-primary"/>Conservação e limpeza</CardTitle></CardHeader>
        <CardContent className="space-y-3"><p className="text-sm leading-6 text-muted-foreground">Os dados operacionais têm prazos definidos e a limpeza é executada automaticamente. Os dados necessários à conta e ao percurso académico mantêm-se enquanto a conta estiver ativa, salvo eliminação pelo utilizador ou outra necessidade aplicável.</p><div className="grid gap-2 md:grid-cols-2">{RETENTION.map(([label, period]) => <div key={label} className="flex items-start justify-between gap-4 rounded-xl border bg-card p-3"><span className="text-xs font-medium">{label}</span><span className="shrink-0 text-right text-xs text-muted-foreground">{period}</span></div>)}</div><p className="text-[11px] leading-5 text-muted-foreground">Os detalhes jurídicos completos, finalidades e critérios de conservação são apresentados na informação de privacidade.</p></CardContent>
      </Card>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card className="premium-card"><CardHeader><CardTitle className="flex items-center gap-2 text-base"><MessageCircle className="h-4 w-4 text-primary"/>Pedido ou esclarecimento</CardTitle></CardHeader><CardContent className="space-y-3"><p className="text-sm leading-6 text-muted-foreground">Se precisares de comunicar uma incorreção, esclarecer o tratamento dos teus dados ou pedir apoio, utiliza o Feedback. Quando estás autenticado, o pedido fica associado internamente à conta e é apresentado ao administrador através do ID Academic Hub.</p><Button asChild variant="outline"><Link to="/feedback">Abrir Feedback</Link></Button></CardContent></Card>
        <Card className="premium-card border-destructive/25"><CardHeader><CardTitle className="flex items-center gap-2 text-base"><Trash2 className="h-4 w-4 text-destructive"/>Eliminar conta</CardTitle></CardHeader><CardContent className="space-y-3"><p className="text-sm leading-6 text-muted-foreground">A eliminação é feita na área Conta e Perfil e exige confirmação explícita. A conta, os registos associados por utilizador e os anexos privados de feedback são removidos pelo procedimento de eliminação.</p><Button asChild variant="outline"><Link to="/conta">Gerir ou eliminar conta</Link></Button></CardContent></Card>
      </div>

      <Card className="premium-card">
        <CardContent className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between"><div><div className="flex items-center gap-2 text-sm font-semibold"><FileText className="h-4 w-4 text-primary"/>Informação de privacidade e contacto</div><p className="mt-1 text-xs leading-5 text-muted-foreground">Consulta a política aplicável, direitos, segurança, responsabilidades e os meios de contacto disponíveis.</p></div><div className="flex flex-wrap gap-2"><Button asChild variant="outline" size="sm"><Link to="/legal">Legal e Privacidade</Link></Button><Button asChild variant="outline" size="sm"><Link to="/seguranca-privacidade">Segurança</Link></Button></div></CardContent>
      </Card>
    </div>
  );
}
