import { Bell, BookOpenCheck, Check, Database, ExternalLink, Monitor, Moon, Palette, ShieldCheck, Sun, UserRound, Volume2 } from "lucide-react";
...
import { notificationSoundEnabled, playAcademicHubAppSound, setNotificationSoundEnabled } from "@/lib/feedbackBeta";
...
export default function SettingsPremium() {
  ...
  const [appSounds, setAppSounds] = useState(() => notificationSoundEnabled());
  const changeTheme = (next: ThemeMode) => { setAppearance({ theme: next }); storeTheme(next); applyTheme(next); };
  const toggleAppSounds = (enabled: boolean) => {
    setAppSounds(enabled);
    setNotificationSoundEnabled(enabled);
    if (enabled) playAcademicHubAppSound("confirm");
  };

  return <div className="settings-premium space-y-6">
    <section className="premium-surface overflow-hidden"><div className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between md:p-6"><div><div className="flex flex-wrap items-center gap-2"><h1 className="text-2xl font-semibold tracking-tight">Definições</h1><span className="rounded-full border bg-muted/40 px-2.5 py-1 text-[10px] font-semibold text-muted-foreground">v{APP_VERSION}</span></div><p className="mt-1.5 max-w-2xl text-sm text-muted-foreground">Preferências da aplicação, aparência, alertas e configuração académica.</p></div><div className="flex flex-wrap gap-2"><Button asChild variant="outline" size="sm"><Link to="/dados-privacidade"><Database className="mr-2 h-4 w-4"/>Os meus dados</Link></Button><Button asChild variant="outline" size="sm"><Link to="/seguranca-privacidade"><ShieldCheck className="mr-2 h-4 w-4"/>Segurança e Privacidade</Link></Button><Button asChild variant="outline" size="sm"><Link to="/conta"><UserRound className="mr-2 h-4 w-4"/>Conta e Perfil</Link></Button><Button asChild variant="outline" size="sm"><Link to="/ajuda"><BookOpenCheck className="mr-2 h-4 w-4"/>Ajuda & Guia</Link></Button></div></div></section>
    ...
    <section className="grid gap-4 lg:grid-cols-2">
      ...
      <Card className="premium-card"><CardHeader className="pb-3"><CardTitle className="flex items-center gap-2 text-base"><Bell className="h-4 w-4 text-primary"/>Alertas e sons dentro da aplicação</CardTitle></CardHeader><CardContent className="space-y-3"><NotificationRow title="Prazos e entregas" description="Avisos visíveis enquanto utilizas o Academic Hub." checked={notifications.deadlines} onCheckedChange={checked=>setNotifications({deadlines:checked})}/><NotificationRow title="Exames e recursos" description="Datas e proximidade das provas dentro da aplicação." checked={notifications.exams} onCheckedChange={checked=>setNotifications({exams:checked})}/><NotificationRow title="Publicação de notas" description="Datas previstas para resultados." checked={notifications.grades} onCheckedChange={checked=>setNotifications({grades:checked})}/><div className="flex items-center justify-between gap-3 rounded-xl border bg-card p-3"><div className="min-w-0"><div className="flex items-center gap-2 text-sm font-medium"><Volume2 className="h-4 w-4 text-primary"/>Sons da aplicação</div><div className="mt-0.5 text-xs text-muted-foreground">Tons discretos para confirmações, avisos, erros e novas notificações enquanto o Academic Hub está aberto.</div></div><Switch checked={appSounds} onCheckedChange={toggleAppSounds}/></div></CardContent></Card>
    </section>
