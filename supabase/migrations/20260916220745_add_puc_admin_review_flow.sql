...
alter table public.puc_catalog_submissions
  drop constraint if exists puc_catalog_submissions_status_check;

alter table public.puc_catalog_submissions
  add constraint puc_catalog_submissions_status_check
  check (status in ('pending','approved','rejected','superseded'));

create index if not exists puc_catalog_submissions_reviewed_by_idx
  on public.puc_catalog_submissions (reviewed_by, resolved_at desc)
...
begin
  ...
  if p_decision not in ('approve','reject') then
    raise exception 'puc_admin_invalid_decision';
  end if;
  ...
  if not found then
    raise exception 'puc_submission_not_found';
  end if;

  if submission_row.status <> 'pending' then
    raise exception 'puc_submission_already_resolved';
  end if;

  if submission_row.kind <> 'correction' then
    raise exception 'puc_admin_only_correction_supported';
  end if;
  ...
  if not found then
    raise exception 'puc_correction_base_unavailable';
  end if;
  ...
  if base_row.is_active is not true then
    ...
    set status = 'superseded',
        ...
        manager_read_at = coalesce(manager_read_at, now()),
        resolved_at = now(),
        resolution_note = coalesce(normalized_note, 'A versão-base deixou de ser a versão ativa.'),
        updated_at = now()
    where id = submission_row.id;
  ...
  insert into public.puc_catalog_entries (
    ...
    submission_row.course_code,
    submission_row.course_name,
    submission_row.academic_year,
    submission_row.edition,
    submission_row.evaluation_model,
    jsonb_build_object(
    ...
    base_row.source_hash,
    base_row.source_page_count,
    next_version,
    true,
    now(),
    now(),
