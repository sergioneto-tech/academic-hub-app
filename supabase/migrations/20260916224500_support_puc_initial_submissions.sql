alter table public.puc_catalog_submissions
  drop constraint if exists puc_catalog_submissions_new_entry_source_check;

alter table public.puc_catalog_submissions
  add constraint puc_catalog_submissions_new_entry_source_check
  check (
    kind <> 'new_entry'
    or (
      source_hash is not null
      and source_hash ~ '^[0-9a-f]{64}$'
      and source_page_count between 1 and 80
      and submitter_declaration_version = 'puc-new-entry-v1'
      and submitter_declared_at is not null
      and submitter_source_confirmed = true
    )
  );

create unique index if not exists puc_catalog_submissions_pending_new_entry_dedupe_idx
  ...
  where kind = 'new_entry' and status = 'pending';
...
begin
  ...
  if submission_row.kind = 'new_entry' then
    select id into active_catalog_id
    from public.puc_catalog_entries
    where course_code = submission_row.course_code
      and academic_year = submission_row.academic_year
      and edition = submission_row.edition
      and is_active = true
    limit 1
    for update;

    if active_catalog_id is not null then
      update public.puc_catalog_submissions
      set status = 'superseded', reviewed_by = caller,
          manager_read_at = coalesce(manager_read_at, now()), resolved_at = now(),
          resolution_note = coalesce(normalized_note, 'Já existe uma versão ativa para esta UC, ano letivo e edição.'),
          updated_at = now()
      where id = submission_row.id;
    ...
    if submission_row.source_hash is null or submission_row.source_page_count is null then
      raise exception 'puc_new_entry_source_required';
    end if;
    ...
    insert into public.puc_catalog_entries (
      ...
      submission_row.source_hash, submission_row.source_page_count, next_version, true, now(),
  ...
  if submission_row.kind <> 'correction' then
    raise exception 'puc_admin_unsupported_submission_kind';
  end if;
...
revoke all on function private.review_puc_catalog_submission_impl(uuid, text, text) from public, anon, authenticated;
